...
class Disparity(torch.nn.Module):
    def __init__(self):
        ...
        self.netImage = torch.nn.Conv2d(
            in_channels=3, out_channels=32, kernel_size=7, stride=2, padding=3
        )
        self.netSemantics = torch.nn.Conv2d(
            in_channels=512, out_channels=512, kernel_size=3, stride=1, padding=1
        )

        for intRow, intFeatures in [
            (0, 32),
            (1, 48),
            (2, 64),
            (3, 512),
            ...
            self.add_module(
                str(intRow) + "x1" + " - " + str(intRow) + "x2",
                Basic("relu-conv-relu-conv", [intFeatures, intFeatures, intFeatures]),
            )
            self.add_module(
                str(intRow) + "x2" + " - " + str(intRow) + "x3",
                Basic("relu-conv-relu-conv", [intFeatures, intFeatures, intFeatures]),
            )
        # end
        ...
        for intCol in [0, 1]:
            ...
            self.add_module(
                "3x" + str(intCol) + " - " + "4x" + str(intCol),
                Downsample([512, 512, 512]),
            )
            self.add_module(
                "4x" + str(intCol) + " - " + "5x" + str(intCol),
                Downsample([512, 512, 512]),
            )
        # end
        ...
        for intCol in [2, 3]:
            self.add_module(
                "5x" + str(intCol) + " - " + "4x" + str(intCol),
                Upsample([512, 512, 512]),
            )
            self.add_module(
                "4x" + str(intCol) + " - " + "3x" + str(intCol),
                Upsample([512, 512, 512]),
            )
            self.add_module(
                "3x" + str(intCol) + " - " + "2x" + str(intCol), Upsample([512, 64, 64])
            )
            self.add_module(
                "2x" + str(intCol) + " - " + "1x" + str(intCol), Upsample([64, 48, 48])
            )
            self.add_module(
                "1x" + str(intCol) + " - " + "0x" + str(intCol), Upsample([48, 32, 32])
            )
        # end
    ...
    def forward(self, tenImage, tenSemantics):
        ...
        tenColumn[3] = self._modules["2x0 - 3x0"](tenColumn[2]) + self.netSemantics(
            tenSemantics
        )
            ...
            if intRow != len(tenColumn) - 1:
                ...
                if tenUp.shape[2] != tenColumn[intRow].shape[2]:
                    tenUp = torch.nn.functional.pad(
                        input=tenUp, pad=[0, 0, 0, -1], mode="constant", value=0.0
                    )
                if tenUp.shape[3] != tenColumn[intRow].shape[3]:
                    tenUp = torch.nn.functional.pad(
                        input=tenUp, pad=[0, -1, 0, 0], mode="constant", value=0.0
                    )
        ...
        return torch.nn.functional.threshold(
            input=self.netDisparity(tenColumn[0]), threshold=0.0, value=0.0
        )
...
netDisparity.load_state_dict(
    {
        strKey.replace("module", "net"): tenWeight
        for strKey, tenWeight in torch.hub.load_state_dict_from_url(
            url="http://content.sniklaus.com/kenburns/network-disparity.pytorch",
            file_name="kenburns-disparity",
        ).items()
    }
)
...
def disparity_estimation(tenImage):
    ...
    tenImage = torch.nn.functional.interpolate(
        input=tenImage, size=(intHeight, intWidth), mode="bilinear", align_corners=False
    )
