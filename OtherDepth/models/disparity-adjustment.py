...
netMaskrcnn = (
    torchvision.models.detection.maskrcnn_resnet50_fpn(pretrained=True).cuda().eval()
)
...
def disparity_adjustment(tenImage, tenDisparity):
    ...
    for intMask in range(objPredictions["masks"].shape[0]):
        ...
        elif objPredictions["labels"][intMask].item() not in [
            1,
            3,
            6,
            7,
        ...
        tenMask = (
            objPredictions["masks"][(intMask + 0) : (intMask + 1), :, :, :] > 0.5
        ).float()
        ...
        for intMerge in range(objPredictions["masks"].shape[0]):
            ...
            elif objPredictions["labels"][intMerge].item() not in [
                2,
                4,
                27,
                28,
            ...
            tenMerge = (
                objPredictions["masks"][(intMerge + 0) : (intMerge + 1), :, :, :] > 0.5
            ).float()
    ...
    tenAdjusted = F.interpolate(
        input=tenDisparity,
        size=(tenImage.shape[2], tenImage.shape[3]),
        mode="bilinear",
        align_corners=False,
    )
    ...
    for tenAdjust in tenMasks:
        ...
        tenPlane = F.max_pool2d(
            input=tenPlane.neg(), kernel_size=3, stride=1, padding=1
        ).neg()
        tenPlane = F.max_pool2d(
            input=tenPlane.neg(), kernel_size=3, stride=1, padding=1
        ).neg()
        ...
        tenAdjusted = ((1.0 - tenAdjust) * tenAdjusted) + (
            tenAdjust
            * tenPlane[
                :, :, int(round(intTop + (0.97 * (intBottom - intTop)))) :, :
            ].max()
        )
    # end
    ...
    return F.interpolate(
        input=tenAdjusted,
        size=(tenDisparity.shape[2], tenDisparity.shape[3]),
        mode="bilinear",
        align_corners=False,
    )
