...
class Refine(nn.Module):
    ...
    def forward(self, tenImage, tenDisparity):
        ...
        if tenUpsample.shape != tenImageThr.shape:
            tenUpsample = F.interpolate(
                input=tenUpsample,
                size=(tenImageThr.shape[2], tenImageThr.shape[3]),
                mode="bilinear",
                align_corners=False,
            )  # not ideal
        ...
        if tenUpsample.shape != tenImageTwo.shape:
            tenUpsample = F.interpolate(
                input=tenUpsample,
                size=(tenImageTwo.shape[2], tenImageTwo.shape[3]),
                mode="bilinear",
                align_corners=False,
            )  # not ideal
        ...
        if tenUpsample.shape != tenImageOne.shape:
            tenUpsample = F.interpolate(
                input=tenUpsample,
                size=(tenImageOne.shape[2], tenImageOne.shape[3]),
                mode="bilinear",
                align_corners=False,
            )  # not ideal
        ...
        return F.threshold(input=tenRefine, threshold=0.0, value=0.0)
...
netRefine.load_state_dict(
    {
        strKey.replace("module", "net"): tenWeight
        for strKey, tenWeight in torch.hub.load_state_dict_from_url(
            url="http://content.sniklaus.com/kenburns/network-refinement.pytorch",
            file_name="kenburns-refinement",
        ).items()
    }
)
