...
class Inpaint(nn.Module):
    def __init__(self):
        ...
        for intCol in [2, 3]:
            ...
            self.add_module(
                "2x" + str(intCol) + " - " + "1x" + str(intCol), Upsample([128, 64, 64])
            )
            self.add_module(
                "1x" + str(intCol) + " - " + "0x" + str(intCol), Upsample([64, 32, 32])
            )
        # end
    ...
    def forward(self, tenImage, tenDisparity, tenShift):
        ...
        tenStd = [
            tenImage.std([1, 2, 3], False, True),
            tenDisparity.std([1, 2, 3], False, True),
        ]
        ...
        tenRender, tenExisting = render_pointcloud(
            tenPoints + tenShift,
            torch.cat([tenImage, tenDisparity, tenContext], 1).view(1, 68, -1),
            objCommon["intWidth"],
            objCommon["intHeight"],
            objCommon["fltFocal"],
        ...
        return {
            ...
            "tenDisparity": torch.nn.functional.threshold(
                input=tenDisparity, threshold=0.0, value=0.0
            ),
        }
...
netInpaint.load_state_dict(
    {
        strKey.replace("module", "net"): tenWeight
        for strKey, tenWeight in torch.hub.load_state_dict_from_url(
            url="http://content.sniklaus.com/kenburns/network-inpainting.pytorch",
            file_name="kenburns-inpainting",
        ).items()
    }
)
